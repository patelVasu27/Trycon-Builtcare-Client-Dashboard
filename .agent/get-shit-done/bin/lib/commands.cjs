...
function determinePhaseStatus(plans, summaries, phaseDir, defaultPending) {
  ...
  try {
    ...
    if (verificationFile) {
      const content =
        platformReadSync(path.join(phaseDir, verificationFile)) || "";
...
function cmdCommitToSubrepo(cwd, message, files, raw) {
  ...
  for (const [repo, repoFiles] of Object.entries(grouped)) {
    ...
    // Get hash
    const hashResult = execGit(["rev-parse", "--short", "HEAD"], {
      cwd: repoCwd,
    });
    const hash = hashResult.exitCode === 0 ? hashResult.stdout : null;
...
function cmdScaffold(cwd, type, options, raw) {
  ...
  switch (type) {
    ...
    case "phase-dir": {
      ...
      const scaffoldPrefix = scaffoldProjectCode
        ? `${scaffoldProjectCode}-`
        : "";
...
function cmdStats(cwd, format, raw) {
  ...
  if (rootHash.exitCode === 0 && rootHash.stdout) {
    ...
    const firstDate = execGit(["show", "-s", "--format=%as", firstCommit], {
      cwd,
    });
