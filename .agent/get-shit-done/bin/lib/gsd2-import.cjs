...
function parseGsd2(gsdDir) {
  ...
  for (const mid of milestoneIds) {
    ...
    const sliceInfos = roadmapContent
      ? parseSlicesFromRoadmap(roadmapContent)
      : [];
    ...
    const slices = sliceInfos.map((info) => {
      ...
      return {
        ...
        plan: hasSDir
          ? readOptional(path.join(sDir, `${info.id}-PLAN.md`))
          : null,
        summary: hasSDir
          ? readOptional(path.join(sDir, `${info.id}-SUMMARY.md`))
          : null,
        research: hasSDir
          ? readOptional(path.join(sDir, `${info.id}-RESEARCH.md`))
          : null,
        context: hasSDir
          ? readOptional(path.join(sDir, `${info.id}-CONTEXT.md`))
          : null,
    ...
    data.milestones.push({
      ...
      title: roadmapContent
        ? (parseMilestoneTitle(roadmapContent) ?? mid)
        : mid,
...
function buildStateMd(phaseMap) {
  ...
  const currentPhaseNum = currentEntry
    ? zeroPad(currentEntry.phaseNum)
    : zeroPad(totalPhases);
  const currentSlug = currentEntry
    ? slugify(currentEntry.slice.title)
    : "complete";
...
function buildPlanningArtifacts(gsd2Data) {
  ...
  artifacts.set(
    "PROJECT.md",
    gsd2Data.projectContent || "# Project\n\n(Migrated from GSD-2)\n",
  );
...
function buildPreview(gsd2Data, artifacts, projectDir) {
  ...
  const totalSlices = gsd2Data.milestones.reduce(
    (s, m) => s + m.slices.length,
    0,
  );
  ...
  const allTasks = gsd2Data.milestones.flatMap((m) =>
    m.slices.flatMap((sl) => sl.tasks),
  );
  ...
  lines.push(
    `  - GSD-2 database state (rebuilt from files on first ${formatGsdSlash("health", resolveRuntime(projectDir))})`,
  );
...
function cmdFromGsd2(args, cwd, raw) {
  ...
  if (!gsdDir) {
    return output(
      { success: false, error: `No .gsd/ directory found in ${projectDir}` },
      raw,
    );
  }
