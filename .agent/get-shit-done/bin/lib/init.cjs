...
function getInitGitState(cwd) {
  ...
  if (info.inside) {
    ...
    try {
      const prefixResult = execGit(["rev-parse", "--show-prefix"], {
        cwd,
        timeout: 5000,
      });
    ...
    if (!resolvedByGitPrefix) {
      ...
      if (rootNorm && cwdNorm) {
        if (rootNorm === cwdNorm) {
          ...
          const relNorm =
            process.platform === "win32" ? rel.replace(/\//g, "\\") : rel;
          inNestedSubdir =
            relNorm !== "" &&
            relNorm !== "." &&
            !relNorm.startsWith("..") &&
    ...
    try {
      ...
      if (patternsFile) {
        result.patterns_path = toPosixPath(
          path.join(phaseInfo.directory, patternsFile),
        );
      }
    } catch {
      /* intentionally empty */
  ...
  try {
    if (fs.existsSync(phasesDir)) {
      ...
      phaseDirCount = fs
        ...
        .filter(
          (entry) => entry.isDirectory() && isDirInMilestone(entry.name),
        ).length;
    }
  } catch {}
...
function cmdInitMilestoneOp(cwd, raw) {
  ...
  if (roadmapPhaseNumbers.length > 0) {
    ...
    for (const num of roadmapPhaseNumbers) {
      ...
      try {
        const hasSummary =
          listPhaseSummaryFiles(path.join(phasesDir, dirName)).length > 0;
    ...
    try {
      ...
      for (const dir of dirs) {
        try {
          const hasSummary =
            listPhaseSummaryFiles(path.join(phasesDir, dir)).length > 0;
          if (hasSummary) completedPhases++;
        } catch {
          /* intentionally empty */
        }
...
function cmdInitManager(cwd, raw) {
  ...
  if (!fs.existsSync(paths.roadmap)) {
    error(
      `No ROADMAP.md found. Run ${formatGsdSlash("new-milestone", _slashRuntime)} first.`,
    );
  }
  if (!fs.existsSync(paths.state)) {
    error(
      `No STATE.md found. Run ${formatGsdSlash("new-milestone", _slashRuntime)} first.`,
    );
  }
  ...
  const _allCompletedPattern =
    /-\s*\[x\]\s*.*Phase\s+(\d+[A-Z]?(?:\.\d+)*)[:\s]/gi;
  let _allMatch;
  ...
  const completedCount = nonBacklogPhases.filter(
...
function buildAgentSkillsBlock(config, agentType, projectRoot) {
  ...
  } = require("./runtime-homes.cjs");
  const runtime = (config && config.runtime) || "claude";
  const globalSkillsBase = require("./runtime-homes.cjs").getGlobalSkillsBase(
    runtime,
  );
  ...
  for (const skillPath of skillPaths) {
    ...
    if (skillPath.startsWith("global:")) {
      ...
      const pathCheck = validatePath(globalSkillMd, globalSkillsBase, {
        allowAbsolute: true,
      });
      if (!pathCheck.safe) {
        process.stderr.write(
...
function buildSkillManifest(cwd, skillsDir = null) {
  ...
  const gsdSkillsInstalled = skills.some((skill) =>
    skill.name.startsWith("gsd-"),
  );
