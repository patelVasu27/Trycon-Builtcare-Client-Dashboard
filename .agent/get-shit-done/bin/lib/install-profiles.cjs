...
function resolveProfile({ modes, manifest, _profilesOverride } = {}) {
  ...
  const normalizedModes = activeModes
    ...
    .map((mode) => mode.trim())
    .filter(Boolean);
  const modesToResolve =
    normalizedModes.length > 0 ? normalizedModes : ["full"];
...
function stageSkillsForProfile(srcDir, resolvedProfile) {
  ...
  const stageDir = fs.mkdtempSync(
    path.join(os.tmpdir(), "gsd-profile-skills-"),
  );
  try {
...
function stageAgentsForProfile(srcAgentsDir, resolvedProfile) {
  ...
  const stageDir = fs.mkdtempSync(
    path.join(os.tmpdir(), "gsd-profile-agents-"),
  );
  try {
    if (
      resolvedProfile.agents instanceof Set &&
      resolvedProfile.agents.size > 0
    ) {
...
function stageSkillsForRuntimeAsSkills(
  ...
  const stageDir = fs.mkdtempSync(
    path.join(os.tmpdir(), "gsd-profile-runtime-skills-"),
  );
  try {
    ...
    for (const entry of entries) {
      ...
      if (resolvedProfile.skills !== "*" && !resolvedProfile.skills.has(stem))
        continue;
      const content = fs.readFileSync(
        path.join(srcCommandsDir, entry.name),
        "utf8",
      );
...
function writeActiveProfile(runtimeConfigDir, profileName) {
  platformWriteSync(
    path.join(runtimeConfigDir, PROFILE_MARKER_NAME),
    profileName + "\n",
  );
}
...
function shouldInstallSkill(skillBaseName, resolvedProfileOrMode) {
  if (
    typeof resolvedProfileOrMode === "object" &&
    resolvedProfileOrMode !== null
  ) {
...
function stageSkillsForMode(srcDir, mode) {
  ...
  const stageDir = fs.mkdtempSync(
    path.join(os.tmpdir(), "gsd-minimal-skills-"),
  );
  try {
    const entries = fs.readdirSync(srcDir, { withFileTypes: true });
    for (const entry of entries) {
