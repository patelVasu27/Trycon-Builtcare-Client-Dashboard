...
function summarizeInstallerMigrationResult(result) {
  ...
  for (const action of actions) {
    ...
    rows.push({
      label: blockedSet.has(action)
        ? "blocked"
        : installerMigrationActionLabel(action),
...
const BUNDLED_GSD_HOOK_FILES = Object.freeze(
  new Set([
...
function buildBlockedErrorMessage(blocked) {
  ...
  for (const [reason, actions] of byReason) {
    lines.push(
      `  - ${actions.length} file${actions.length === 1 ? "" : "s"}: ${reason}`,
    );
