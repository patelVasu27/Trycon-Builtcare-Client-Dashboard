...
function intelDiff(planningDir) {
  ...
  for (const [_key, filename] of Object.entries(INTEL_FILES)) {
    ...
    if (currentHash && !prevHashes[filename]) {
      ...
      currentHash &&
      prevHashes[filename] &&
      currentHash !== prevHashes[filename]
    ) {
    ...
    if (data.entries && typeof data.entries === "object") {
      ...
      if (key === "files") {
        ...
        for (const ep of entryPaths) {
          if (!fs.existsSync(ep)) {
            warnings.push(
              `${filename}: entry path "${ep}" does not exist on disk`,
            );
          }
        }
      }
      ...
      if (key === "deps") {
        for (const [depName, entry] of Object.entries(data.entries)) {
          ...
          if (missing.length > 0) {
            warnings.push(
              `${filename}: "${depName}" missing fields: ${missing.join(", ")}`,
            );
          }
        }
      }
...
function intelExtractExports(filePath) {
  ...
  if (allMatches.length > 0) {
    ...
    for (const line of lines) {
      ...
      const keyMatch =
        trimmed.match(/^(\w+)\s*[,}:]/) || trimmed.match(/^(\w+)$/);
      if (keyMatch) {
