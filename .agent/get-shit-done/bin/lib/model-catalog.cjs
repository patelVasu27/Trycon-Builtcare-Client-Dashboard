...
const _catalogCandidates = [
  ...
  process.env.GSD_MODEL_CATALOG
    ? path.resolve(process.env.GSD_MODEL_CATALOG)
    : null,
].filter(Boolean);
...
const MODEL_ALIAS_MAP = Object.fromEntries(
  Object.entries(catalog.runtimeTierDefaults.claude).map(([tier, entry]) => [
    tier,
    entry?.model,
  ]),
);
...
const RUNTIMES_WITH_REASONING_EFFORT = new Set(
  Object.entries(catalog.runtimeTierDefaults)
    .filter(([, tiers]) =>
      Object.values(tiers).some((entry) => entry && entry.reasoning_effort),
    )
    .map(([runtime]) => runtime),
);
...
function getAgentToModelMapForProfile(normalizedProfile) {
  const profile = VALID_PROFILES.includes(normalizedProfile)
    ? normalizedProfile
    : "balanced";
  const out = {};
