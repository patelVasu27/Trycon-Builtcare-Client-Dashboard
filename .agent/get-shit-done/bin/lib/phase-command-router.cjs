...
function routePhaseCommand({ phase, args, cwd, raw, error }) {
  ...
  const UNSUPPORTED = {
    "list-plans":
      "phase list-plans is SDK-only. Use: gsd-sdk query phase.list-plans ...",
    "list-artifacts":
      "phase list-artifacts is SDK-only. Use: gsd-sdk query phase.list-artifacts ...",
    scaffold:
      "phase scaffold is routed through the top-level scaffold command.",
  };
        ...
        for (let i = 2; i < args.length; i++) {
          ...
          if (token === "--id") {
            ...
            return {
              ok: false,
              errorKind: "InvalidArgs",
              message: `phase add does not support ${token}`,
            };
          } else {
            descArgs.push(token);
          }
        ...
        if (descFlagIdx !== -1) {
          ...
          if (!rawDescriptions || rawDescriptions.startsWith("--")) {
            return {
              ok: false,
              errorKind: "InvalidArgs",
              message: "--descriptions must be a JSON array",
            };
          }
          try {
      ...
      insert: (_ctx) => {
        if (args.includes("--dry-run")) {
          return {
            ok: false,
            errorKind: "InvalidArgs",
            message: "phase insert does not support --dry-run",
          };
        }
      ...
      remove: (_ctx) => {
        ...
        for (const token of removeArgs) {
          ...
          if (token.startsWith("--")) {
            return {
              ok: false,
              errorKind: "InvalidArgs",
              message: `phase remove does not support ${token}`,
            };
          }
        ...
        if (positional.length !== 1) {
          return {
            ok: false,
            errorKind: "InvalidArgs",
            message: "phase remove accepts exactly one phase number",
          };
        }
