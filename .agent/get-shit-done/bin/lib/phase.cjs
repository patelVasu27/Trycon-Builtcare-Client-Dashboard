...
function describeNonCanonicalPlans(dirFiles, matchedFiles) {
  ...
  const offenders = dirFiles.filter(
    (f) => looksLikePlanFile(f) && !matched.has(f),
  );
  ...
  try {
    ...
    if (fs.existsSync(phasesDir)) {
      ...
      const dirPattern = new RegExp(
        `^(?:[A-Z]{1,6}-)?${escapeRegex(normalized)}\\.(\\d+)`,
      );
      for (const dir of dirs) {
...
function cmdPhasePlanIndex(cwd, phase, raw) {
  ...
  for (const planFile of planFiles) {
    ...
    const hasSummary =
      completedPlanIds.has(planId) ||
      completedPlanIds.has(extractCanonicalPlanId(planFile));
  ...
  const canonicalToId = new Map(
    rawPlans.map((p) => [extractCanonicalPlanId(p.id), p.id]),
  );
  ...
  if (visited < rawPlans.length) {
    const cycleNodes = rawPlans
      .filter((p) => !level.has(p.id))
      .map((p) => p.id);
    error(
...
function cmdPhaseInsert(cwd, afterPhase, description, raw) {
  ...
  const { decimalPhase, dirName } = withPlanningLock(cwd, () => {
    ...
    const isBulletStyle =
      !headingMatch && bulletPattern.test(content) && !roadmapHasHeadingPhases;
    ...
    if (isBulletStyle) {
      ...
      updatedContent =
        rawContent.slice(0, insertIdx) +
        bulletEntry +
        rawContent.slice(insertIdx);
    } else {
  ...
  if (fs.existsSync(statePath)) {
    readModifyWriteStateMd(
      ...
      (stateContent) => {
        ...
        const ofMatch = stateContent.match(
          /(\bof\s+)(\d+)(\s*(?:\(|phases?))/i,
        );
        if (ofMatch) {
  ...
  if (fs.existsSync(roadmapPath)) {
    withPlanningLock(cwd, () => {
      ...
      if (fs.existsSync(reqPath)) {
        ...
        const traceabilityHeadingMatch = reqContent.match(
          /^#{1,6}\s+Traceability\b/im,
        );
        ...
        let tableMatch;
        while (
          (tableMatch = tableRowPattern.exec(traceabilitySection)) !== null
        ) {
  ...
  if (fs.existsSync(statePath)) {
    readModifyWriteStateMd(
      ...
      (stateContent) => {
        ...
        const completedRaw = stateExtractField(
          stateContent,
          "Completed Phases",
        );
        if (completedRaw) {
  ...
  try {
    ...
    if (fs.existsSync(configPath)) {
      ...
      const autoPruneEnabled =
        rawConfig.workflow && rawConfig.workflow.auto_prune_state === true;
      if (autoPruneEnabled && fs.existsSync(statePath)) {
        const { cmdStatePrune } = require("./state.cjs");
        cmdStatePrune(
          cwd,
          { keepRecent: "3", dryRun: false, silent: true },
          true,
        );
        autoPruned = true;
      }
