  ...
  if (explicitFlags.size > 0) {
    ...
    for (const slug of normalizedDefaults.values) {
      if (!KNOWN_REVIEWER_SLUGS.includes(slug)) {
        warnings.push(
          `unknown reviewer slug in review.default_reviewers: ${slug}`,
        );
      } else {
    ...
    if (undetected.length > 0) {
      infos.push(
        `configured reviewers not detected on this host: ${undetected.join(", ")}`,
      );
    }
    selected = knownDefaults.filter((slug) => detected.has(slug));
    if (selected.length === 0 && errors.length === 0) {
      errors.push(
        "all configured default reviewers are unavailable on this host",
      );
    }
  } else {
    selected = [...detected];
