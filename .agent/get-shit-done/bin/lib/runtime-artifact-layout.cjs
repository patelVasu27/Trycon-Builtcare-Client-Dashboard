...
function findInstallSourceRoot(runtimeConfigDir) {
  ...
  throw new Error(
    `findInstallSourceRoot: could not locate commands/gsd from ${__dirname}`,
  );
}
...
function findAgentsSourceRoot(runtimeConfigDir) {
    ...
    if (fs.existsSync(markerPath)) {
      try {
        ...
        if (src && fs.existsSync(src)) {
          ...
          const agentsCandidate = path.resolve(
            path.dirname(src),
            "..",
            "agents",
          );
  ...
  throw new Error(
    `findAgentsSourceRoot: could not locate agents/ from ${__dirname}`,
  );
}
...
function commandsKind(destSubpath, prefix, configDir) {
  return {
    ...
    prefix,
    stage: (resolved) =>
      stageSkillsForProfile(findInstallSourceRoot(configDir), resolved),
  };
}
...
function agentsKind(destSubpath, prefix, configDir) {
  return {
    ...
    prefix,
    stage: (resolved) =>
      stageAgentsForProfile(findAgentsSourceRoot(configDir), resolved),
  };
}
  ...
  return {
    ...
    stage: (resolved) => {
      ...
      return stageSkillsForRuntimeAsSkills(
        findInstallSourceRoot(configDir),
        resolved,
        wrappedConverter,
        prefix,
      );
...
function resolveRuntimeArtifactLayout(runtime, configDir, scope = "global") {
  ...
  if (!ALLOWED_RUNTIMES.has(runtime)) {
    throw new TypeError(
      `Unknown runtime: '${runtime}' — add to runtime-artifact-layout.cjs table`,
    );
  }
