...
function routeStateCommand({ state, args, cwd, raw, parseNamedArgs, error }) {
  ...
  routeCjsCommandFamily({
    ...
    subcommands: [
      "load",
      "complete-phase",
      ...STATE_SUBCOMMANDS.filter((s) => s !== "load"),
    ],
    ...
    handlers: {
      ...
      json: sdkHandler("state.json", [], args.slice(1), null, () =>
        state.cmdStateJson(cwd, raw),
      ),
      get: sdkHandler("state.get", args.slice(2), args.slice(1), null, () =>
      ...
      "planned-phase": sdkHandler(
        ...
        null,
        () => {
          const { phase: p, plans } = parseNamedArgs(args, [
            "phase",
            "name",
            "plans",
          ]);
      ...
      validate: sdkHandler("state.validate", [], args.slice(1), null, () =>
        state.cmdStateValidate(cwd, raw),
      ),
      sync: sdkHandler("state.sync", args.slice(2), args.slice(1), null, () => {
      ...
      prune: sdkHandler(
        ...
        null,
        () => {
          const { "keep-recent": keepRecent, "dry-run": dryRun } =
            parseNamedArgs(args, ["keep-recent"], ["dry-run"]);
          state.cmdStatePrune(
            cwd,
            { keepRecent: keepRecent || "3", dryRun: !!dryRun },
            raw,
          );
        },
      ),
      ...
      "milestone-switch": sdkHandler(
        ...
        null,
        () => {
          const { milestone, name } = parseNamedArgs(args, [
            "milestone",
            "name",
          ]);
