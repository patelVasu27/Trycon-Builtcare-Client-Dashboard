...
function readPristineHashes(patchesDir) {
  ...
  try {
    ...
    if (
      parsed &&
      typeof parsed.pristine_hashes === "object" &&
      parsed.pristine_hashes !== null
    ) {
...
function computeUserAddedLines(backupContent, pristineContent) {
  ...
  return backupLines.filter(
    (line) => isSignificantLine(line) && !pristineSet.has(line),
  );
}
...
function main() {
  ...
  const files = walk(opts.patchesDir).filter(
    (f) => !f.endsWith("backup-meta.json"),
  );
  ...
  const driftedResults = results.filter(
    (r) => r.reason === REASON.OK_PRISTINE_DRIFT_DETECTED,
  );
