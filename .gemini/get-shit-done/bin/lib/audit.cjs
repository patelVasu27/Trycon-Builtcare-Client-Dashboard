...
function scanQuickTasks(planDir) {
  ...
  for (const entry of entries) {
    ...
    let safeTaskDir;
    try {
      safeTaskDir = requireSafePath(taskDir, planDir, "quick task dir", {
        allowAbsolute: true,
      });
    } catch {
      continue;
    ...
    try {
      const summaryFiles = fs
        ...
        .filter(
          (e) =>
            e.isFile() &&
            (e.name === "SUMMARY.md" || e.name.endsWith("-SUMMARY.md")),
        );
...
function scanThreads(planDir) {
  ...
  for (const entry of files) {
    ...
    let safeFilePath;
    try {
      safeFilePath = requireSafePath(filePath, planDir, "thread file", {
        allowAbsolute: true,
      });
    } catch {
      continue;
    ...
    if (!status) {
      const bodyStatusMatch = content.match(
        /##\s*Status:\s*(OPEN|IN PROGRESS|IN_PROGRESS)/i,
      );
      if (bodyStatusMatch) {
...
function scanTodos(planDir) {
  ...
  for (const entry of displayFiles) {
    ...
    let safeFilePath;
    try {
      safeFilePath = requireSafePath(filePath, planDir, "todo file", {
        allowAbsolute: true,
      });
    } catch {
      continue;
...
function scanSeeds(planDir) {
  ...
  for (const entry of files) {
    ...
    let safeFilePath;
    try {
      safeFilePath = requireSafePath(filePath, planDir, "seed file", {
        allowAbsolute: true,
      });
    } catch {
      continue;
...
function scanUatGaps(planDir) {
  ...
  for (const dir of dirs) {
    ...
    for (const file of files.filter(
      ...
      let safeFilePath;
      try {
        safeFilePath = requireSafePath(filePath, planDir, "UAT file", {
          allowAbsolute: true,
        });
      } catch {
        continue;
      ...
      const pendingMatches = (
        content.match(/result:\s*(?:pending|\[pending\])/gi) || []
      ).length;
...
function scanContextQuestions(planDir) {
  ...
  for (const dir of dirs) {
    ...
    for (const file of files.filter(
      (f) => f.includes("-CONTEXT") && f.endsWith(".md"),
    )) {
      const filePath = path.join(phaseDir, file);

      let safeFilePath;
      try {
        safeFilePath = requireSafePath(filePath, planDir, "CONTEXT file", {
          allowAbsolute: true,
        });
      } catch {
        continue;
      ...
      if (fm.open_questions) {
        if (Array.isArray(fm.open_questions) && fm.open_questions.length > 0) {
          questions = fm.open_questions.map((q) =>
            sanitizeForDisplay(String(q).slice(0, 200)),
          );
        }
      }
      ...
      if (questions.length === 0) {
        ...
        if (oqMatch) {
          ...
          if (oqBody && oqBody.length > 0 && !/^\s*none\s*$/i.test(oqBody)) {
            ...
            questions = items
              .slice(0, 3)
              .map((q) => sanitizeForDisplay(q.slice(0, 200)));
          }
        }
      }
...
function formatAuditReport(auditResult) {
  ...
  if (counts.todos > 0) {
    const realTodos = items.todos.filter(
