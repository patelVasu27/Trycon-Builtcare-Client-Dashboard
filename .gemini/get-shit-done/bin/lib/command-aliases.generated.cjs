...
const STATE_SUBCOMMANDS = STATE_COMMAND_ALIASES.map(
  (entry) => entry.subcommand,
);
const VERIFY_SUBCOMMANDS = VERIFY_COMMAND_ALIASES.map(
  (entry) => entry.subcommand,
);
const INIT_SUBCOMMANDS = INIT_COMMAND_ALIASES.map((entry) => entry.subcommand);
const PHASE_SUBCOMMANDS = PHASE_COMMAND_ALIASES.map(
  (entry) => entry.subcommand,
);
const PHASES_SUBCOMMANDS = PHASES_COMMAND_ALIASES.map(
  (entry) => entry.subcommand,
);
const VALIDATE_SUBCOMMANDS = VALIDATE_COMMAND_ALIASES.map(
  (entry) => entry.subcommand,
);
const ROADMAP_SUBCOMMANDS = ROADMAP_COMMAND_ALIASES.map(
  (entry) => entry.subcommand,
);
