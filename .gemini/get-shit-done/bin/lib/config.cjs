...
} = require("./review-reviewer-selection.cjs");
...
const SHIP_PR_BODY_SECTION_KEYS = new Set([
  "heading",
  "enabled",
  "source",
  "fallback",
...
function validateShipPrBodySections(value) {
  ...
  value.forEach((section, index) => {
    ...
    if (!hasContent) {
      error(
        `${prefix}. Provide at least one of source, fallback, or template.`,
      );
    }
...
function setConfigValue(cwd, keyPath, parsedValue) {
  ...
  return withPlanningLock(cwd, () => {
    ...
    try {
      ...
      error(
        "Failed to read config.json: " + err.message,
...
function cmdConfigSet(cwd, keyPath, value, raw) {
  ...
  if (
    keyPath === "context" &&
    !VALID_CONTEXT_VALUES.includes(String(parsedValue))
  ) {
    error(
      `Invalid context value '${value}'. Valid values: ${VALID_CONTEXT_VALUES.join(", ")}`,
    );
  }
  ...
  if (
    keyPath === "workflow.drift_action" &&
    !VALID_DRIFT_ACTIONS.includes(String(parsedValue))
  ) {
    error(
  ...
  if (keyPath === "workflow.drift_threshold") {
    if (
      ...
      error(
        `Invalid workflow.drift_threshold '${value}'. Must be a positive integer.`,
      );
    }
  }
  ...
  if (keyPath === "workflow.post_planning_gaps") {
    if (typeof parsedValue !== "boolean") {
      error(
        `Invalid workflow.post_planning_gaps '${value}'. Must be a boolean (true or false).`,
      );
    }
  }
  ...
  if (keyPath === "git.create_tag") {
    if (typeof parsedValue !== "boolean") {
      error(
        `Invalid git.create_tag '${value}'. Must be a boolean (true or false).`,
      );
    }
  }
