...
const CONFIG_DEFAULTS = {
  ...
  milestone_branch_template:
    CANONICAL_CONFIG_DEFAULTS.git.milestone_branch_template,
...
function loadConfig(cwd, options = {}) {
  const activeWorkstream = Object.prototype.hasOwnProperty.call(
    options,
    "workstream",
  )
    ? options.workstream
  ...
  if (ws) {
    ...
    try {
      ...
      const { parsed: rootNormalized, normalizations: rootNorms } =
        normalizeLegacyKeys(rootParsed);
  ...
  try {
    ...
    {
      const { parsed: normalized, normalizations } =
        normalizeLegacyKeys(fileData);
    ...
    const parsed = rootParsed
      ? _deepMergeConfig(rootParsed, fileData)
      : fileData;
    ...
    return {
      ...
      post_planning_gaps:
        get("post_planning_gaps", {
          section: "workflow",
          field: "post_planning_gaps",
      ...
      tdd_mode:
        get("tdd_mode", { section: "workflow", field: "tdd_mode" }) ?? false,
      mvp_mode:
        get("mvp_mode", { section: "workflow", field: "mvp_mode" }) ?? false,
      text_mode:
        get("text_mode", { section: "workflow", field: "text_mode" }) ??
        defaults.text_mode,
      auto_advance:
        get("auto_advance", { section: "workflow", field: "auto_advance" }) ??
        false,
      _auto_chain_active:
        get("_auto_chain_active", {
          section: "workflow",
          field: "_auto_chain_active",
        }) ?? false,
    ...
    try {
      ...
      return {
        ...
        nyquist_validation:
          globalDefaults.nyquist_validation ?? defaults.nyquist_validation,
        post_planning_gaps:
          globalDefaults.post_planning_gaps ??
          globalDefaults.workflow?.post_planning_gaps ??
          defaults.post_planning_gaps,
        parallelization:
          globalDefaults.parallelization ?? defaults.parallelization,
        text_mode: globalDefaults.text_mode ?? defaults.text_mode,
        resolve_model_ids:
          globalDefaults.resolve_model_ids ?? defaults.resolve_model_ids,
        context_window:
          globalDefaults.context_window ?? defaults.context_window,
        subagent_timeout:
          globalDefaults.subagent_timeout ?? defaults.subagent_timeout,
...
function extractCanonicalPlanId(filename) {
  ...
  if (
    phaseIdx >= 0 &&
    phaseIdx + 1 < parts.length &&
    tokenRe.test(parts[phaseIdx + 1])
  ) {
...
function gitWorktreeInfoInternal(cwd) {
  try {
    const insideResult = execGit(["rev-parse", "--is-inside-work-tree"], {
      cwd,
      timeout: 5000,
    });
    ...
    const rootResult = execGit(["rev-parse", "--show-toplevel"], {
      cwd,
      timeout: 5000,
    });
  ...
  try {
    ...
    if (versionOverride) {
      ...
      if (!sectionMatch) {
        ...
        const hasVersionedMilestones = /^#{1,3}\s+.*v\d+\.\d+/im.test(
          roadmapContent,
        );
        ...
        const sectionStart = sectionMatch.index;
        const headingLevel = sectionMatch[1].match(/^(#{1,3})\s/)[1].length;
        const restContent = roadmapContent.slice(
