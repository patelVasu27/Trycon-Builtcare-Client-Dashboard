...
function formatGapTable(rows) {
  ...
  const body = rows
    .map((r) => {
      const tick =
        r.status === "Covered" ? "\u2713 Covered" : "\u2717 Not covered";
...
function readGate(cwd) {
  ...
  try {
    ...
    if (
      raw &&
      raw.workflow &&
      typeof raw.workflow.post_planning_gaps === "boolean"
    ) {
...
function runGapAnalysis(cwd, phaseDir) {
  if (!readGate(cwd)) {
    return {
      ...
      table: "",
      summary:
        "workflow.post_planning_gaps disabled — skipping post-planning gap analysis",
      counts: { total: 0, covered: 0, uncovered: 0 },
    };
  }

  const absPhaseDir = path.isAbsolute(phaseDir)
    ? phaseDir
    : path.join(cwd, phaseDir);

  const reqPath = planningPaths(cwd).requirements;
  const reqMd = fs.existsSync(reqPath) ? fs.readFileSync(reqPath, "utf-8") : "";
  const reqItems = parseRequirements(reqMd).map((r) => ({
    ...r,
  ...
  const dItems = parseDecisions(ctxMd).map((d) => ({
    ...d,
    source: "CONTEXT.md",
  }));
