...
function disabledResponse() {
  return {
    disabled: true,
    message:
      "graphify is not enabled. Enable with: gsd-tools config-set graphify.enabled true",
  };
}
...
function execGraphify(cwd, args, options = {}) {
  ...
  return {
    ...
    reason:
      result.exitCode === 0 ? GRAPHIFY_REASON.OK : GRAPHIFY_REASON.EXIT_NONZERO,
  };
}
...
function checkGraphifyVersion() {
  ...
  if (!versionStr) {
    return {
      version: null,
      compatible: null,
      warning: "Could not determine graphify version",
    };
  }
  ...
  const warning = compatible
    ...
    : "graphify version " +
      versionStr +
      " is outside tested range >=0.4.0,<1.0";
...
function seedAndExpand(graph, term, maxHops = 2) {
  ...
  return {
    ...
    seeds: new Set(seeds.map((n) => n.id)),
  };
}
...
function applyBudget(result, budgetTokens) {
  ...
  return {
    ...
    trimmed:
      omitted > 0
        ? `[${omitted} edges omitted, ${unreachable} nodes unreachable]`
        : null,
...
function graphifyStatus(cwd) {
  ...
  if (!fs.existsSync(graphPath)) {
    return {
      exists: false,
      message: "No graph built yet. Run graphify build to create one.",
    };
  }
  ...
  const lastBuildAutoUpdate = fs.existsSync(statusPath)
    ? safeReadJson(statusPath)
    : null;
  const autoUpdateStale =
    lastBuildAutoUpdate &&
    (lastBuildAutoUpdate.status === "failed" ||
      lastBuildAutoUpdate.status === "running");
...
function graphifyDiff(cwd) {
  ...
  // Diff nodes
  const currentNodeMap = Object.fromEntries(
    (current.nodes || []).map((n) => [n.id, n]),
  );
  const snapshotNodeMap = Object.fromEntries(
    (snapshot.nodes || []).map((n) => [n.id, n]),
  );

  const nodesAdded = Object.keys(currentNodeMap).filter(
    (id) => !snapshotNodeMap[id],
  );
  const nodesRemoved = Object.keys(snapshotNodeMap).filter(
    (id) => !currentNodeMap[id],
  );
  const nodesChanged = Object.keys(currentNodeMap).filter(
    (id) =>
      snapshotNodeMap[id] &&
      JSON.stringify(currentNodeMap[id]) !==
        JSON.stringify(snapshotNodeMap[id]),
  );

  // Diff edges (keyed by source+target+relation)
  const edgeKey = (e) =>
    `${e.source}::${e.target}::${e.relation || e.label || ""}`;
  ...
  const edgesAdded = Object.keys(currentEdgeMap).filter(
    (k) => !snapshotEdgeMap[k],
  );
  const edgesRemoved = Object.keys(snapshotEdgeMap).filter(
    (k) => !currentEdgeMap[k],
  );
  ...
  return {
    nodes: {
      ...
      changed: nodesChanged.length,
    },
    edges: {
      added: edgesAdded.length,
      removed: edgesRemoved.length,
      changed: edgesChanged.length,
    },
