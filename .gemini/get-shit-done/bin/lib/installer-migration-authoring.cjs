...
function requireNonEmptyString(record, field, source) {
  if (typeof record[field] !== "string" || record[field].trim() === "") {
    throw new Error(
      `migration record must include a non-empty ${field}: ${source}`,
    );
  }
}
...
function requireStringArray(record, field, source) {
  if (
    ...
    throw new Error(
      `migration record ${field} must be a non-empty string array: ${source}`,
    );
  }
}
...
function validateInstallerMigrationRecord(record, source) {
  ...
  if (typeof record.destructive !== "boolean") {
    throw new Error(
      `migration record must declare destructive as a boolean: ${displaySource}`,
    );
  }
  ...
  if (typeof record.plan !== "function") {
    throw new Error(
      `migration record must include a plan function: ${displaySource}`,
    );
  }
...
function actionSource(migration, action) {
  const migrationId =
    migration && typeof migration.id === "string" ? migration.id : "<unknown>";
  const relPath =
    action && typeof action.relPath === "string" ? action.relPath : "<unknown>";
...
function validateSafeRelPath(relPath, migration, actionType) {
  ...
  if (path.isAbsolute(normalized) || path.win32.isAbsolute(normalized)) {
    throw new Error(
      `migration action ${actionType} relPath must stay inside configDir: ${source}`,
    );
  }
...
function validateInstallerMigrationActions(actions, migration) {
  ...
  for (const action of actions) {
    ...
    if (typeof action.type !== "string" || action.type.trim() === "") {
      throw new Error(
        `migration action must include a non-empty type: ${migration.id}`,
      );
    }
    ...
    if (
      ...
      (typeof migration.runtimeContract !== "string" ||
        migration.runtimeContract.trim() === "")
    ) {
      throw new Error(
