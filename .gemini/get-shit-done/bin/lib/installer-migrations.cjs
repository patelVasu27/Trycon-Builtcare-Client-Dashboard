...
function readJsonIfPresent(filePath, fallback) {
  ...
  try {
    ...
    if (fallback === STRICT_JSON) {
      throw new Error(
        `invalid installer migration state JSON: ${filePath}: ${error.message}`,
      );
    }
    return fallback;
  }
...
function readInstallState(configDir) {
  const state = readJsonIfPresent(
    path.join(configDir, INSTALL_STATE_NAME),
    STRICT_JSON,
  );
  ...
  return {
    ...
    appliedMigrations: Array.isArray(state.appliedMigrations)
...
function normalizeRelPath(relPath) {
  ...
  if (path.isAbsolute(normalized) || path.win32.isAbsolute(normalized)) {
    throw new Error(
      `migration action relPath must stay inside configDir: ${relPath}`,
    );
  }
...
function migrationChecksum(migration) {
  ...
  const serializable = {
    ...
    plan:
      typeof migration.plan === "function" ? migration.plan.toString() : null,
  };
  ...
  return fs
    ...
    .flatMap((fileName) => {
      ...
      return records.map((record) =>
        validateInstallerMigrationRecord(record, source),
      );
    });
}
      ...
      return () => {
        ...
        if (failures.length > 0) {
          const releaseError = new Error(
            `failed to release installer migration lock: ${lockPath}`,
          );
      ...
      if (error && error.code === "EEXIST") {
        ...
        if (Date.now() - started >= timeoutMs) {
          const holderInfo = lockData
            ...
            : "";
          throw new Error(
            `installer migration lock is held: ${lockPath}${holderInfo}`,
          );
        }
        sleepSync(
          Math.min(50, Math.max(1, timeoutMs - (Date.now() - started))),
        );
        continue;
      }
      throw error;
...
function planInstallerMigrations({
  ...
  const pending = scopedMigrations.filter(
  ...
  const classify = (relPath) => {
    ...
    if (!classifications.has(normalized)) {
      classifications.set(
        normalized,
        classifyArtifact(configDir, normalized, manifest),
      );
    }
...
function uniqueActionMigrationIds(actions) {
  return [
    ...new Set(actions.map((action) => action.migrationId).filter(Boolean)),
  ];
}
...
function applyInstallerMigrationPlan({
  ...
  if (Array.isArray(plan.blocked) && plan.blocked.length > 0) {
    throw new Error(
      `migration plan has ${plan.blocked.length} blocked action(s)`,
    );
  }

  const appliedAt = now();
  const runId = migrationRunId(appliedAt);
  const journalRelPath = path.posix.join(
    "gsd-migration-journal",
    `${runId}.json`,
  );
  const journalPath = path.join(configDir, journalRelPath);
  const rollbackRootRelPath = path.posix.join(
    "gsd-migration-journal",
    `${runId}-rollback`,
  );
  const rollbackRoot = path.join(configDir, rollbackRootRelPath);
  const backupRootRelPath = path.posix.join(
    "gsd-migration-journal",
    `${runId}-backups`,
  );
  ...
  try {
    ...
    for (const action of plan.actions) {
      ...
      const { normalized, fullPath } = ensureInsideConfig(
        configDir,
        action.relPath,
      );
      if (!fs.existsSync(fullPath)) {
      ...
      if (
        ...
        journal.actions.push(
          journalAction(
            action,
            action.type === "record-baseline" ? "recorded" : "preserved",
          ),
        );
        continue;
      }
      ...
      if (action.type === "backup-and-remove") {
        const backupRelPath =
          action.backupRelPath ||
          path.posix.join(backupRootRelPath, normalized);
    ...
    for (const id of journal.appliedMigrationIds) {
      if (!applied.has(id)) {
        ...
        nextApplied.push({
          ...
          checksum:
            action && action.migrationChecksum
              ? action.migrationChecksum
              : null,
        });
      }
    }
...
function markPendingMigrationsApplied({
  ...
  if (
    !plan ||
    !Array.isArray(plan.pendingMigrationIds) ||
    plan.pendingMigrationIds.length === 0
  ) {
    return [];
  }
  const appliedAt = now();
...
function runInstallerMigrations({
  ...
  const releaseLock = acquireInstallMigrationLock(configDir, {
  ...
  try {
    ...
    if (plan.actions.length === 0) {
      const appliedMigrationIds = markPendingMigrationsApplied({
        configDir,
        plan,
        now,
      });
