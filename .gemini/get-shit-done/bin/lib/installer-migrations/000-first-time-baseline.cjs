...
function isStaleGsdLookingPath(relPath) {
  ...
  const parts = relPath.split("/");
  if (
    (parts[0] === "skills" || parts[0] === "agents") &&
    parts[1] &&
    parts[1].startsWith("gsd-")
  ) {
    return true;
  }
  return false;
...
module.exports = {
  ...
  description:
    "Classify existing install surfaces before destructive installer migrations run.",
  introducedIn: "1.50.0",
  scopes: ["global", "local"],
  destructive: false,
    ...
    for (const relPath of scanBaselineFiles(configDir, runtime)) {
      ...
      if (isUserOwnedBaselinePath(relPath)) {
        actions.push({
          ...
          relPath,
          reason:
            "known user-owned artifact preserved by first-time migration baseline",
      ...
      if (
        ...
        actions.push({
          ...
          relPath,
          reason:
            "existing manifest-managed file included in first-time migration baseline",
        });
        continue;
      }
      ...
      if (isKnownGeneratedAgentPath(relPath, runtime)) {
        actions.push({
          ...
          relPath,
          reason:
            "known installer-generated agent included in first-time migration baseline",
      ...
      if (isStaleGsdLookingPath(relPath)) {
        actions.push({
          ...
          relPath,
          reason:
            "GSD-looking file is not proven manifest-managed and needs explicit user choice",
      ...
      actions.push({
        ...
        relPath,
        reason:
          "unknown install-surface file preserved by first-time migration baseline",
        classification: artifact.classification,
        originalHash: artifact.originalHash,
        currentHash,
      });
