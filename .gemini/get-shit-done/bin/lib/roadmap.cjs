...
function cmdRoadmapGetPhase(cwd, phaseNum, raw) {
  ...
  try {
    ...
    if (exactSource) {
      const exactMilestone = searchPhaseInContent(
        milestoneContent,
        exactSource,
        phaseNum,
      );
      ...
      const exactFull = searchPhaseInContent(
        fullContent,
        exactSource,
        phaseNum,
      );
...
function cmdRoadmapAnalyze(cwd, raw) {
  ...
  while ((match = phasePattern.exec(content)) !== null) {
    ...
    try {
      const dirMatch = _phaseDirNames.find((d) =>
        phaseTokenMatches(d, normalized),
      );

      if (dirMatch) {
        const counts = countPhasePlansAndSummaries(
          path.join(phasesDir, dirMatch),
        );
...
function cmdRoadmapAnnotateDependencies(cwd, phaseNum, raw) {
  ...
  if (!fs.existsSync(roadmapPath)) {
    output(
      { updated: false, reason: "ROADMAP.md not found" },
      raw,
      "no roadmap",
    );
    return;
  ...
  for (const planFile of phaseInfo.plans) {
    const planPath = path.join(
      path.resolve(cwd, phaseInfo.directory),
      planFile,
    );
    try {
  ...
  if (planData.length === 0) {
    output(
      { updated: false, reason: "could not read plan frontmatter" },
      raw,
      "no frontmatter",
    );
    return;
  ...
  withPlanningLock(cwd, () => {
    ...
    const nextPhaseOffset = restAfterHeader
      .slice(1)
      .search(/\n#{2,4}\s+Phase\s+\d/i);
    const phaseEnd =
      nextPhaseOffset >= 0 ? phaseStart + 1 + nextPhaseOffset : content.length;
    ...
    for (const line of listLines) {
      ...
      const planEntry = planId
        ? planData.find((p) => p.planId === planId)
        : null;
    ...
    for (let i = 0; i < sortedWaves.length; i++) {
      ...
      if (sortedWaves.length > 1) {
        const dep =
          i > 0 ? ` *(blocked on Wave ${sortedWaves[i - 1]} completion)*` : "";
    ...
    const nextContent =
      content.slice(0, phaseStart) + newPhaseSection + content.slice(phaseEnd);
