...
function formatHookCommandForRuntime(command, opts = {}) {
  return hookCommandNeedsPowerShellCallOperator(opts)
    ? `& ${command}`
    : command;
}
...
function projectManagedHookCommand({
  ...
  const normalizedScriptPath =
    platform === "win32" ? scriptPath.replace(/\\/g, "/") : scriptPath;
...
function managedHookSurfaceSet(surface = "settings-json") {
  return (
    MANAGED_HOOK_BASENAMES_BY_SURFACE[surface] ||
    MANAGED_HOOK_BASENAMES_BY_SURFACE["settings-json"]
  );
}
...
function managedHookCommandSurfaceSet(
...
function isManagedHookCommand(commandText, opts = {}) {
  ...
  const managedBasenames = managedHookCommandSurfaceSet(
    surface,
...
function projectCodexHookTomlCommand({
  absoluteRunner,
  scriptPath,
  platform = process.platform,
}) {
...
function projectPathActionProjection({
  ...
  if (isWin32) {
    ...
    const bashTargetDir = escapeSingleQuotedShellLiteral(
      String(targetDir).replace(/\\/g, "/"),
    );
    shellActions = [
      {
        label: "PowerShell",
...
function projectPersistentPathExportActions({
  targetDir,
  platform = process.platform,
}) {
...
function formatSdkPathDiagnostic({ shimDir, platform, runDir }) {
  ...
  if (shimDir) {
    ...
    actionLines.push(
      "Could not locate a writable PATH directory to install the shim.",
    );
...
function platformWriteSync(filePath, content, opts = {}) {
  const { content: normalized, encoding } = normalizeContent(
    filePath,
    content,
    opts,
  );
