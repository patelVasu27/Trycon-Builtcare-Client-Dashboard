    ...
    (content) => {
      ...
      if (currentPlan >= totalPlans) {
        ...
        if (useCompoundFormat) {
          ...
          content =
            stateReplaceField(content, "Plan", planDisplayValue) || content;
        } else {
...
function cmdStateSnapshot(cwd, raw) {
  ...
  const currentPhase =
    fmScalar("current_phase") ?? stateExtractField(body, "Current Phase");
  const currentPhaseName =
    fmScalar("current_phase_name") ??
    stateExtractField(body, "Current Phase Name");
  const totalPhasesRaw =
    fmScalar("total_phases") ?? stateExtractField(body, "Total Phases");
  const currentPlan =
    fmScalar("current_plan") ?? stateExtractField(body, "Current Plan");
  const totalPlansRaw =
    fmScalar("total_plans_in_phase") ??
    stateExtractField(body, "Total Plans in Phase");
  const status = fmScalar("status") ?? stateExtractField(body, "Status");
  const progressRaw =
    fmScalar("progress") ?? stateExtractField(body, "Progress");
  const lastActivity =
    fmScalar("last_activity") ?? stateExtractField(body, "Last Activity");
  const lastActivityDesc =
    fmScalar("last_activity_desc") ??
    stateExtractField(body, "Last Activity Description");
  const pausedAt =
    fmScalar("paused_at") ?? stateExtractField(body, "Paused At");
...
function buildStateFrontmatter(bodyContent, cwd) {
  ...
  const sessionSectionMatch = bodyContent.match(
    /##\s*Session\s*\n([\s\S]*?)(?=\n##|$)/i,
  );
  const sessionBodyScope = sessionSectionMatch
    ? sessionSectionMatch[1]
    : bodyContent;
  const stoppedAt =
      ...
      if (fs.existsSync(phasesDir)) {
        ...
        if (!cached) {
          ...
          for (const dir of allMatchingDirs) {
            ...
            if (!seenPhaseNums.has(key)) {
              ...
              try {
                ...
                if (
                  fs.statSync(candidate).mtimeMs > fs.statSync(existing).mtimeMs
                ) {
                  seenPhaseNums.set(key, dir);
                }
              } catch {
          ...
          for (const dir of phaseDirs) {
            ...
            const { planCount, summaryCount, completed } =
              scanPhasePlans(phaseDir);
...
function cmdStateJson(cwd, raw) {
  ...
  if (
    existingFm &&
    shouldPreserveExistingProgress(existingFm.progress, built.progress)
  ) {
  ...
  try {
    ...
    if (positionPattern.test(body)) {
      newBody = body.replace(
        positionPattern,
        (_m, header) => `${header}${resetPositionBody}`,
      );
    } else {
  ...
  if (currentPhase && fs.existsSync(phasesDir)) {
    ...
    try {
      ...
      if (phaseDir) {
        ...
        const { planCount: diskPlans, summaryCount: diskSummaries } =
          scanPhasePlans(phaseDirPath);
...
function cmdStatePrune(cwd, options, raw) {
  ...
  const currentPhaseRaw = stateExtractField(
  ...
  if (dryRun) {
    ...
    const totalPruned = result.archivedSections.reduce(
      (sum, s) => sum + s.count,
      0,
    );
    emit(
...
function cmdStateCompletePhase(cwd, raw, overridePhase) {
  ...
  const existingCurrentPhaseRaw =
    stateExtractField(content, "Current Phase") || "";
  const existingCurrentPhaseMatch = String(existingCurrentPhaseRaw).match(
    /(\d+[A-Z]?(?:\.\d+)*)/i,
  );
  const existingCurrentPhase = existingCurrentPhaseMatch
    ? existingCurrentPhaseMatch[1]
    : null;
