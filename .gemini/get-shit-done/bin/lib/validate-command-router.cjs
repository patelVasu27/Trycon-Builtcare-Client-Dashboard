    ...
    handlers: {
      ...
      health: () => {
        ...
        verify.cmdValidateHealth(
          cwd,
          { repair: repairFlag, backfill: backfillFlag },
          raw,
        );
      },
      agents: sdkHandler("validate.agents", args.slice(2), args.slice(1), () =>
      ...
      context: () => {
        ...
        if (opts["context-window"] === null) {
          error(
            "--context-window <integer> is required for `validate context`",
          );
          return;
        }
        const {
          ...
          STATES,
        } = require("./context-utilization.cjs");
        ...
        try {
          ...
          const flag = /tokensUsed/.test(e.message)
            ? "--tokens-used"
            : "--context-window";
          error(
            `${flag} must be a non-negative integer (window > 0), got the values supplied`,
          );
          return;
        }
        const result = {
          ...classified,
          recommendation: RECOMMENDATIONS[classified.state],
        };
        if (args.includes("--json")) {
          outputFn(result, raw);
        } else {
          const lines = [
            `Context utilization: ${result.percent}% (${result.state})`,
          ];
