  ...
  for (const phaseRoot of phaseRoots) {
    try {
      ...
      for (const dir of dirs) {
        ...
        const phaseLabel = path
          .relative(planBase, phasePath)
          .replace(/\\/g, "/");
        ...
        for (const sid of summaryIds) {
          if (!planIds.has(sid)) {
            warnings.push(
              `Summary ${sid}-SUMMARY.md in ${phaseLabel} has no matching PLAN.md`,
            );
          }
        }
        ...
        for (const plan of plans) {
          ...
          if (!fm.wave) {
            warnings.push(
              `${phaseLabel}/${plan}: missing 'wave' in frontmatter`,
            );
          }
        }
      }
...
function cmdValidateHealth(cwd, options, raw) {
  ...
  if (!fs.existsSync(planBase)) {
    addIssue(
      ...
      `Run ${slash("new-project")} to initialize`,
    );
    output(
      {
        status: "broken",
  ...
  if (!fs.existsSync(projectPath)) {
    addIssue(
      ...
      `Run ${slash("new-project")} to create`,
    );
  } else {
  ...
  if (!fs.existsSync(roadmapPath)) {
    addIssue(
      ...
      `Run ${slash("new-milestone")} to create roadmap`,
    );
  }
  ...
  if (!fs.existsSync(statePath)) {
    ...
    const phaseRefs = [
      ...stateContent.matchAll(/[Pp]hase\s+(\d+[A-Z]?(?:\.\d+)*)/g),
    ].map((m) => m[1]);
    ...
    try {
      if (fs.existsSync(roadmapPath)) {
        ...
        const all = [
          ...roadmapRaw.matchAll(/#{2,4}\s*Phase\s+(\d+[A-Z]?(?:\.\d+)*)/gi),
        ];
  ...
  if (fs.existsSync(configPath)) {
    try {
      ...
      if (
        configParsed.workflow &&
        configParsed.workflow.ai_integration_phase === undefined
      ) {
        addIssue(
          "warning",
          "W016",
  ...
  if (fs.existsSync(roadmapPath)) {
    ...
    const uncheckedPattern =
      /-\s*\[\s\]\s*\*{0,2}Phase\s+(\d+[A-Z]?(?:\.\d+)*)[:\s*]/gi;
    let um;
            ...
            for (const ver of missingFromRegistry) {
              try {
                ...
                if (!milestonesContent.trim()) {
                  ...
                  const headerMatch = milestonesContent.match(
                    /^(#{1,3}\s+[^\n]*\n\n?)/,
                  );
                  if (headerMatch) {
...
function cmdVerifyCodebaseDrift(cwd, raw) {
  ...
  try {
    ...
    const config = loadConfig(cwd);
    const threshold =
      Number.isInteger(config?.workflow?.drift_threshold) &&
      config.workflow.drift_threshold >= 1
        ? config.workflow.drift_threshold
        : 3;
    const action =
      config?.workflow?.drift_action === "auto-remap" ? "auto-remap" : "warn";
