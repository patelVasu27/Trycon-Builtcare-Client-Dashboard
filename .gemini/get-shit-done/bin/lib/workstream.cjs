...
function cmdWorkstreamCreate(cwd, name, options, raw) {
  ...
  if (!fs.existsSync(baseDir)) {
    error(
      `.planning/ directory not found — run ${formatGsdSlash("new-project", resolveRuntime(cwd))} first`,
    );
  }
          ...
          output(
            {
              ...
              message:
                "Invalid migrate-name — must contain at least one alphanumeric character",
            },
            raw,
          );
          return;
