...
function readWorktreeList(repoRoot, deps = {}) {
  ...
  const listResult = execGit(["worktree", "list", "--porcelain"], {
    cwd: repoRoot,
  });
...
function snapshotWorktreeInventory(repoRoot, options = {}, deps = {}) {
  ...
  const listed = listLinkedWorktreePaths(repoRoot, {
    execGit: deps.execGit || execGitDefault,
  });
  if (!listed.ok) {
    return {
      ok: false,
...
function normalizeCleanupManifestEntry(entry) {
  ...
  const expectedBase =
    typeof entry.expected_base === "string" ? entry.expected_base : "";
...
function executeWorktreeWaveCleanupPlan(plan, deps = {}) {
  ...
  for (let i = 0; i < entries.length; i += 1) {
    ...
    const branchCheck = execGit(
      ["-C", entry.worktree_path, "rev-parse", "--abbrev-ref", "HEAD"],
      { cwd: plan.repoRoot },
    );
    if (
      !gitResultOk(branchCheck) ||
      branchCheck.stdout.trim() !== entry.branch
    ) {
    ...
    const mergeBase = execGit(["merge-base", "HEAD", entry.branch], {
      cwd: plan.repoRoot,
    });
    if (
      !gitResultOk(mergeBase) ||
      mergeBase.stdout.trim() !== entry.expected_base
    ) {
      result.status = "blocked";
    ...
    const rescuedRelPaths = rescueSummaryArtifacts(
      entry.worktree_path,
      plan.repoRoot,
      deps,
    );
    ...
    let remove = execGit(
      ["worktree", "remove", entry.worktree_path, "--force"],
      { cwd: plan.repoRoot },
    );
    if (!gitResultOk(remove)) {
      ...
      execGit(["worktree", "unlock", entry.worktree_path], {
        cwd: plan.repoRoot,
      });
    ...
    const branchDelete = execGit(["branch", "-D", entry.branch], {
      cwd: plan.repoRoot,
    });
...
function reapOrphanWorktrees(repoRoot, deps = {}) {
  ...
  const reapMtimeGuardMs =
    deps.reapMtimeGuardMs !== undefined
      ? deps.reapMtimeGuardMs
      : REAP_MTIME_GUARD_MS;
  ...
  if (gitResultOk(defaultBranchResult)) {
    ...
    const branchName = defaultBranchResult.stdout
      .trim()
      .replace(/^origin\//, "");
    const r = execGit(["rev-parse", `refs/remotes/origin/${branchName}`], {
      cwd: repoRoot,
    });
    ...
    const configResult = execGit(["config", "--get", "init.defaultBranch"], {
      cwd: repoRoot,
    });
    ...
    const headSymref = execGit(["symbolic-ref", "--quiet", "--short", "HEAD"], {
      cwd: repoRoot,
    });
  ...
  const listedResult = execGit(["worktree", "list", "--porcelain"], {
    cwd: repoRoot,
  });
  ...
  for (const entryName of entries) {
    ...
    const worktreePath =
      path.basename(resolvedGitFile) === ".git"
    ...
    const removeResult = execGit(
      ["worktree", "remove", gitKnownPath, "--force"],
      { cwd: repoRoot },
    );
