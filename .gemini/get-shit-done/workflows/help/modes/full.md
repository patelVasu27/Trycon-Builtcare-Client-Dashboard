...
- Saves timestamped note to `.planning/notes/` (or `C:/Users/tbc21/antigravity/TRYCON-BUILTCARE-Client-Dashboard/.gemini/notes/` globally)
...
Every topic output starts with a `**Topic:** \`<alias>\` → \`<heading>\` _(scope: full | compact)_`preamble so resolved routing is visible. See`get-shit-done/workflows/help/modes/topic.md` for the full alias table. Unknown topics print the recognized list.
