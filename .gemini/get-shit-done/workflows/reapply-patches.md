...
if [ -z "$PATCHES_DIR" ]; then
  if [ -d "$HOME/.config/kilo/gsd-local-patches" ]; then
    ...
    PATCHES_DIR="C:/Users/tbc21/antigravity/TRYCON-BUILTCARE-Client-Dashboard/.gemini/gsd-local-patches"
  fi
fi
...
- **Mechanical drift** — path substitutions (e.g. `/Users/xxx/.claude/` → `C:/Users/tbc21/antigravity/TRYCON-BUILTCARE-Client-Dashboard/.gemini/`), variable additions (`${GSD_WS}`, `${AGENT_SKILLS_*}`), error handling additions (`|| true`)
