...
function formatGsdState(s) {
  ...
  const phasesStr =
    s.nextPhases && s.nextPhases.length > 0 ? s.nextPhases.join("/") : null;
  ...
  if (s.activePhase) {
    ...
    parts.push(
      stage ? `Phase ${s.activePhase} ${stage}` : `Phase ${s.activePhase}`,
    );
...
function runStatusline() {
  ...
  process.stdin.on("end", () => {
    ...
    try {
      ...
      const claudeDir =
        process.env.CLAUDE_CONFIG_DIR || path.join(homeDir, ".gemini");
